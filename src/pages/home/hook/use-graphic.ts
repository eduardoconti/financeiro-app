...
export const useGraphic = create<IUseGraphic>((set) => ({
  ...
  fetchGenereal: async () => {
    ...
    set((state) => ({
      ...state,
      general: general.map((e) => {
        return { ...e, medianExpenses, medianEarnings };
      }),
    }));
  },
  unplanned: [
  ...
  fetchUnplannedExpenses: async () => {
    ...
    const {
      data: { months },
    } = await graphicService.unplannedExpenses();
    ...
    set((state) => ({
      ...
      unplanned: months.map((e) => {
        return {
          ...e,
          total: Money.toFloat(e.total),
          median: Money.toFloat(median),
        };
      }),
    }));
  },
