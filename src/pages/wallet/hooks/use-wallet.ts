...
export interface IUseWallet {
  ...
  fetchWallets: () => void
  insertWallet: (walletRequest: WalletRequestDTO) => Promise<SuccessResponseData<WalletResponseDTO>>
  updateWallet: (walletRequest: WalletRequestDTO) => Promise<SuccessResponseData<WalletResponseDTO>>
...
export const useWallet = create<IUseWallet>((set) => ({
  ...
  insertWallet: async (walletRequest: WalletRequestDTO) => {
    ...
    set((state) => ({ wallets: [...state.wallets, data.data].sort((a, b) => (a.descricao > b.descricao) ? 1 : ((b.descricao > a.descricao) ? -1 : 0)) }))
    return data;
  },
