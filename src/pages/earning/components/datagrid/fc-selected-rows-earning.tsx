...
import { useDataGridEarning, useEarning, useFormEarning } from "@pages/earning/hooks";
...
function calculateSelectedRows(selectedRows: number[], earnings: EarningResponseDTO[]): number {
  const total = earnings.reduce((value: number, earning: EarningResponseDTO): number => {
...
function FlagPayedButton(props: {
  ...
  const onClick = (async () => {
    ...
    setAlert(setCreatedAlert(HttpStatus.OK, 'Success', 'Receitas alteradas com sucesso'));
...
function DeleteEarningButton(props: any) {
  ...
  const onClick = async () => {
    ...
    setAlert(setCreatedAlert(HttpStatus.OK, 'Success', 'Receitas excluidas com sucesso'));
  };
...
function AddNextMonthButton() {
  ...
  const onClick = async () => {
    try {
      ...
      setAlert(setCreatedAlert(HttpStatus.OK, 'Success', 'Receitas inseridas com sucesso'));
    } catch (error: any) {
      setAlert(setCreatedAlert(error.status, error.detail, error.title));
    } finally {
      setSpin(false);
